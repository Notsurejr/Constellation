...
Constellation.settings = (function () {
  ...
  async function applyEditors() {
    ...
    Constellation.chat.setPrompts({ roleplay: sys, project: proj });   // tie the change to THIS chat
    Constellation.chat.persist();                                       // and save it with the chat now
  }
  ...
  async function refreshPresets() {
    ...
    none.value = ''; none.textContent = '— Select a preset —';
    ...
    sel.value = selectedPreset || '';
  }

  async function loadPreset() {
    const id = $('presetSelect').value;
    if (!id) { selectedPreset = null; return; }
    let p;
  ...
  async function savePreset() {
    ...
    // reuse an existing preset with the same name; otherwise create new
  ...
  async function deletePreset() {
    ...
    if (id === selectedPreset) selectedPreset = null;
    $('presetName').value = '';
    await refreshPresets();
  }
